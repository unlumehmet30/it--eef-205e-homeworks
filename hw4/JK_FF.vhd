...
architecture Structural of JK_FF is
    [cite_start]-- Master: JK Latch (Ödev Yönergesi [cite: 59])
    component JK_latch
    Port ( J, K, C : in STD_LOGIC; Q, Q_bar : inout STD_LOGIC);
end component;

    -- Slave: SR Latch (Kilitlenmek için)
component SR_latch
...
signal Clk_inv : STD_LOGIC;
    ...
    -- Düzeltme: Ara sinyaller
signal Q_internal, Q_bar_internal : STD_LOGIC;
begin
    -- Clock Inverter
    Inv_C: NAND_gate port map (a => Clk, b => Clk, c => Clk_inv);

    -- Master: JK Latch (Clk = '1' iken çalışır)
    Master: JK_latch port map (
        J => J, K => K, C => Clk,
        Q => Q_m, Q_bar => Q_bar_m
    );

    -- Slave: SR Latch (Clk = '0' iken Master'ın verisini kilitler)
    -- Master Q -> Slave S
    -- Master Q_bar -> Slave R
    Slave: SR_latch port map (
        S => Q_m, R => Q_bar_m, C => Clk_inv,
        Q => Q_internal, Q_bar => Q_bar_internal
    );
